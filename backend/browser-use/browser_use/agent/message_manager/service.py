...
class MessageManager:
	...
	def get_condensed_messages(self) -> list[BaseMessage]:
		"""Get a condensed message list for the planner/fast LLM.

		Returns a lightweight version of the messages with:
		- System message intact
		- State message with no screenshots (text-only DOM)
		- Only the last 3 history items to reduce token usage
		"""
		...
		for msg in messages:
			if isinstance(msg, SystemMessage):
				...
				# For non-system messages, strip image content parts
				if isinstance(msg.content, list):
					text_parts = [part for part in msg.content if isinstance(part, ContentPartTextParam)]
					if text_parts:
						# Create a copy with only text parts
						new_msg = msg.__class__(content=text_parts)
						condensed.append(new_msg)
				elif isinstance(msg.content, str):
					# Truncate very long text messages for the planner
					content = msg.content
					if len(content) > 15000:
						content = content[:15000] + '\n... [Content condensed for fast planning]'
						new_msg = msg.__class__(content=content)
						condensed.append(new_msg)
					else:
						condensed.append(msg)
